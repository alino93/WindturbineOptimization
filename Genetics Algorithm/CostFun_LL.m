...
WT=ss(A,B,C,D);
Controller=tf([1, x(1)],[1, x(2)])*tf(x(3),[1, 0]);
CLoop = lft( WT([1:3 3],1:3) , Controller );

Ta=Rho/2*pi*R^3*0.4857/7.1*v.^2;

y = lsim(CLoop(1:3,1:2),[v,Ta],time,[1;1;0;0;0]);            % Generate C.L. response
